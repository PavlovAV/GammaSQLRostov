...
-- Description:	Заполнение табличной части отходов закрытия смены БДМ
-- =============================================

CREATE PROCEDURE [dbo].[FillDocCloseShiftPMWastes] 
...
BEGIN
	...
	DECLARE @Spools TABLE (NomenclatureID uniqueidentifier, CharacteristicID uniqueidentifier, Quantity decimal(15,5), 
	...
	INSERT INTO @Spools exec dbo.FillDocCloseShiftPMSpools @PlaceID, @ShiftID, @CloseDate
		...
		FROM
			...
			(
				SELECT a.*, ISNULL(c.[1CSpecificationID], b.[1CSpecificationID]) AS [1CSpecificationID], ISNULL(c.Period, b.Period) AS Period,
					1 AS Coefficient, ROW_NUMBER() OVER (PARTITION BY a.NomenclatureID, a.CharacteristicID ORDER BY ISNULL(c.Period, b.Period) DESC) 
					AS RowNumber
				FROM
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [Wrapper]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [OperatorRW]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [OperatorBDM]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [TechnologSGB]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [Engineer]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [Dispetcher]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [QualityInspector]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [OperatorConverting]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [TechnologSGI]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [WarehouseOperator]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [Loader]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [Baler]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [Viewer]
    AS [dbo];


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[FillDocCloseShiftPMWastes] TO [PalletRepacker]
    AS [dbo];
