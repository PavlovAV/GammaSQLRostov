...
BEGIN
	...
	BEGIN
		IF @StateID = 3
			SET @ResultMessage = 'Внимание, данный тамбур Ограниченно годный!';
		ELSE
			SET @ResultMessage = 'Данный тамбур не является Годным!';
		--IF @PlaceGroupID = 1
		--BEGIN
		--	SET @BlockCreation = 0;
		--END
	...
	ELSE
		IF EXISTS(SELECT * FROM SourceSpools WHERE (Unwinder1Spool = @ProductID OR Unwinder2Spool = @ProductID OR Unwinder3Spool = @ProductID))
			BEGIN
				...
				SET @ResultMessage = 'Данный тамбур уже установлен на раскат на переделе '+ ISNULL(@Place,'') + '!'
				SET @BlockCreation = 1
			END
